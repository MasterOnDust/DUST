...
def plot_emission_time_series(
    dset, title=None, ax=None, varName=None, auto_locator=False, **plot_kwargs
):
...
def plot_emission_sensitivity(
    ...
    **kwargs
    ...
    if isinstance(info_dict, dict):
        ...
        info_dict = {"Version": dset.attrs["source"], "ldirect": dset.attrs["ldirect"]}
        create_info_str(ax, info_dict, info_loc)

    return im


def plot_log_anomaly(
    dataarray,
    linthresh,
    vmin,
    vmax,
    ...
    **kwargs
    ...
    norm = mpl.colors.SymLogNorm(
        linthresh=linthresh, linscale=linscale, vmin=vmin, vmax=vmax, base=base
    )
    if lower_bound == None:
...
def create_info_str(ax, info_dict, loc):
    ...
    anc_text = AnchoredText(
        info_str, loc=loc, bbox_transform=ax.transAxes, prop=dict(size=8)
    )
    ax.add_artist(anc_text)


def mpl_base_map_plot_xr(
    dataarray,
    ax=None,
    plotting_method="pcolormesh",
    log=True,
    ...
    **kwargs
    ...
    if plotting_method == "pcolormesh":
        im = dataarray.plot.pcolormesh(
            ax=ax, norm=norm, cmap=cmap, add_colorbar=False, **kwargs
        )
    elif plotting_method == "contourf":
        im = dataarray.plot.contourf(
            ax=ax, norm=norm, cmap=cmap, add_colorbar=False, **kwargs
        )
