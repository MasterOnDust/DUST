...
TODO:
    * Fix commandline parser
    * Determine type of output Wet/Dry etc, can actually determine based on whats in the 
      NETCDF file. 
    * Make sure that output netcdf has the right chunk size. 
    * Fix units of output
...
if __name__ == "__main__":
    ...
    parser.add_argument('--psdfraction','--pf',help='particle size distrubtion fraction')
    ...
    for i in range(len(ds.pointspec)):
        out_data[i] = ds.spec001_mr.sel(pointspec=i) * flexdust_ds.Emission
    ...
    out_ds = xr.Dataset({f_name : out_data, 'surface_sensitvity':ds.spec001_mr, 'RELEND':ds.RELEND, 'RELSTART':ds.RELSTART, 
                        ...
                        ,'RELCOM':ds.RELCOM}, attrs=ds.attrs)
    ds.close()
    ...
    out_ds.attrs['history'] = out_ds.attrs['history'] + ' {}'.format(terminal_input)
    ...
    outFile_name = os.path.join(outpath,f_name + '_' + spec_com + out_ds.iedate +'-'+ out_ds.ibdate + '.nc')
